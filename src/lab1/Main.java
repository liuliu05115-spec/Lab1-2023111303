...
public class Main {
    ...
    public static String generateNewText(String inputText) {
        ...
        for (int i = 0; i < words.length - 1; i++) {
            ...
            List<String> bridges = new ArrayList<>();
            if (!w1.isEmpty() && !w2.isEmpty() && currentGraph.getWords().contains(w1)
                    && currentGraph.getWords().contains(w2) && adj.containsKey(w1)) {
        ...
        while (!pq.isEmpty()) {
            ...
            if (adj.containsKey(u)) {
                for (Map.Entry<String, Integer> neighbor : adj.get(u).entrySet()) {
                    ...
                    if (!visited.contains(v) && dist.get(u) != Integer.MAX_VALUE
    ...
    public static Double calPageRank(String word) {
        ...
        if (!currentGraph.getWords().contains(word))
            return 0.0;
